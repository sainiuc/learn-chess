...
exports = module.exports = function(io){
    io.on('connection',function(socket){
        ...
        io.emit("greetings","Welcome to Learn Chess Server");
       ...
       socket.on("joinRoom",joinRoom)
        ...
        function joinRequestTo(name){
          fromname = users.filter(user=>user.id == socket.id)[0].name;
          console.log(fromname + ' sent request to ' + name);
          for(var i=0;i<users.length;i++){
            if(users[i].name===name){
              socket.broadcast.to(users[i].room).emit("joinRequestFrom", fromname, socket.id);
              break;
            }
          }
        }
        ...
        function joinRoom(room){
          var oppname = users.filter(u=>u.id == socket.id)[0].name;
          console.log(oppname + " joined room " + room);
          socket.broadcast.to(room).emit("sendMessage", oppname + " just joined this room.");
          if(room){
            socket.join(room);
        ...
        function sendName(name){
          ...
          for(var i=0;i<users.length;i++){
            if(users[i].name===name){
              isNameValid = false;
              socket.emit('nameError','Name already exists, please try again');
              return;
            }
          }
          if(isNameValid){
